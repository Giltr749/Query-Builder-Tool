...
const dbWifi = new sqlite3.Database('./src/files/databases/dbWifi.sqlite3');
dbWifi.on('error', err => {
    console.log(err);
    process.exit(-1);
});

const dbBt = new sqlite3.Database('./src/files/databases/dbBt.sqlite3');
dbBt.on('error', err => {
    console.log(err);
    process.exit(-1);
});
...
export const createTables = async (req, res, next) => {
    fs.readdir('./src/files/parsed', (err, files) => {
        if (err) throw err;
        else {
