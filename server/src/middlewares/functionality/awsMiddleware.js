...
export const getS3File = async (req, res, next) => {
    ...
    try {
        ...
        const downloaded = fs.readdirSync('./src/files/downloads')
        for await (let key of fileKeys) {
            if (!downloaded.includes(key.replaceAll('/', '_'))) {
                ...
                if (filesList.includes(key)) {
                    ...
                    const writeStream = fs.createWriteStream(`./src/files/downloads/${key.replaceAll('/', '_')}`);
        ...
        for await (let file of toDownload) {
            if (!downloaded.includes(file.replaceAll('/', '_'))) {
                const options = {
                    ...
                    const fileStream = s3.getObject(options).createReadStream();
                    const writeStream = fs.createWriteStream(`./src/files/downloads/${file.replaceAll('/', '_')}`);
